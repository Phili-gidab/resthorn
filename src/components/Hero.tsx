...
/* ============================================================
   ...
   The poster paints first and the footage cross-fades over it the
   moment it can play. A whisper of green shade holds the lower-left
   where the statement stands; the standfirst answers from the
   lower-right as an editorial deck. Film grain and a slight grade
   keep the footage from reading raw. The cursor leans the film one
   way and the statement the other; the elevation cue drips toward
   the ticker. Reduced-motion / data-saver: poster frame, everything
   already in place.
   ============================================================ */
...
export default function Hero({ lng, base }: { lng: Locale; base: string }) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      intro
        ...
        .from('.hb-deck', { y: 18, opacity: 0, duration: 0.7, ease: 'power3.out' }, 1.15)
        .from('.hero-cta > *', { y: 20, opacity: 0, duration: 0.75, stagger: 0.09, ease: 'power3.out' }, 1.25)
        .from('.hero-cue', { opacity: 0, duration: 0.8, ease: 'power2.out' }, 1.6)
      ...
      /* --- cursor parallax: the film leans one way, the statement the other --- */
      if (window.matchMedia('(pointer: fine)').matches) {
        const fx = gsap.quickTo('.hero-film', 'x', { duration: 0.9, ease: 'power3' })
        const fy = gsap.quickTo('.hero-film', 'y', { duration: 0.9, ease: 'power3' })
        const bx = gsap.quickTo('.hero-body', 'x', { duration: 1.1, ease: 'power3' })
        const by = gsap.quickTo('.hero-body', 'y', { duration: 1.1, ease: 'power3' })
        move = (e: MouseEvent) => {
  ...
  return (
    <section className="hero" data-elev="3900" ref={root}>
      ...
      {/* the statement and the deck */}
      <div className="hero-body">
        <div className="shell hb-grid">
      ...
      <div className="hero-cue" aria-hidden="true">
        <span className="hc-elev">≈3,900 m</span>
        <span className="hc-line"><span className="hc-dot" /></span>
        <span className="hc-word" lang={lng}>{t('hero.scroll')}</span>
      </div>
